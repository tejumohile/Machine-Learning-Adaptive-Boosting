	...
	private void displayIterationOutput(BufferedWriter bufferedWriter) {
		try {
			if(itrOutput.isLeftPositive())
				...
				bufferedWriter.write("The selected weak classifier: I(x > "+itrOutput.getWeakClassifierValue()+")");
			bufferedWriter.newLine();
			bufferedWriter.write("The G error value of h: "+itrOutput.getErrorEpsilon());
			bufferedWriter.newLine();
			bufferedWriter.write("The weight C+: "+itrOutput.getcPlus());
			bufferedWriter.newLine();
			bufferedWriter.write("The weight C-: "+itrOutput.getcMinus());
			bufferedWriter.newLine();
			...
			for (int j = 0; j < itrOutput.getNewPValues().length; j++) {
				bufferedWriter.write(" "+itrOutput.getNewPValues()[j]);
			}
			bufferedWriter.newLine();
			bufferedWriter.write("The values f(x) for each one of the examples.");
			double[] finalValueOfF = itrOutput.getFinalValueOfF();
			for (int j = 0; j < inputValues.getNumberOfExamples(); j++) {
				bufferedWriter.write(" "+finalValueOfF[j]);
			}
			bufferedWriter.newLine();
			bufferedWriter.write("The error of the boosted classifier: ");
			bufferedWriter.write(Double.toString(itrOutput.getValueOfEt()));
			bufferedWriter.newLine();
			bufferedWriter.write("The bound on Et: ");
			bufferedWriter.write(Double.toString(itrOutput.getBoundOnEt()));
			bufferedWriter.newLine();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
